...
def test_vehicle_manager_get_svm_details_delegates_to_api():
    ...
    expected = SVMDetails(image_bytes=b"cached")
    with patch.object(
        manager.api, "get_svm_details", return_value=expected
    ) as mock_get:
        result = manager.get_svm_details(vehicle.id)
    assert result is expected
    mock_get.assert_called_once_with(manager.token, vehicle)
...
def test_vehicle_manager_request_svm_capture_delegates_to_api():
    ...
    expected = SVMDetails(image_bytes=b"fresh")
    with patch.object(
        manager.api, "request_svm_capture", return_value=expected
    ) as mock_capture:
        result = manager.request_svm_capture(vehicle.id, acknowledged_warning=True)
    assert result is expected
    mock_capture.assert_called_once_with(manager.token, vehicle, True)
