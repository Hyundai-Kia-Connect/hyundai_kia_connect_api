...
class TestCheckActionStatus:
    def test_success(self):
        api = _make_api()
        api.session.post.return_value = _FakeResponse(json_data={"status": "SUCCESS"})
    ...
    def test_failed(self):
        api = _make_api()
        api.session.post.return_value = _FakeResponse(json_data={"status": "ERROR"})
        vehicle = _make_vehicle()
        token = _make_token()
    ...
    def test_pending(self):
        api = _make_api()
        api.session.post.return_value = _FakeResponse(json_data={"status": "PENDING"})
        vehicle = _make_vehicle()
        token = _make_token()
    ...
    def test_empty_response_returns_unknown(self):
        api = _make_api()
        api.session.post.return_value = _FakeResponse(json_data=None)
        vehicle = _make_vehicle()
        token = _make_token()
