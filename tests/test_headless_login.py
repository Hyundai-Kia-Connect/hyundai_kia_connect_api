...
def test_login_refresh_token_regex_boundary():
    """Verify the 48-char alphanumeric regex correctly classifies passwords."""
    # 48-char uppercase+digit -> refresh_token
    assert re.match(
        r"^[A-Z0-9]{48}$", "NWIXYJNKZJMTZJE3MI01ZWI4LWI0NWETZJQ0NJI1OTFMOTC3"
    )
    # Plaintext password -> NOT a refresh_token
    assert not re.match(r"^[A-Z0-9]{48}$", "MyPassword123!")
    # Lowercase -> NOT a refresh_token (must be uppercase)
    assert not re.match(
        r"^[A-Z0-9]{48}$", "nwixyjkzjmtzje3mi01zwi4lwi0nwetzjQ0nji1otfmotc3"
    )
    # 47 chars -> NOT a refresh_token
    assert not re.match(
        r"^[A-Z0-9]{48}$", "NWIXYJNKZJMTZJE3MI01ZWI4LWI0NWETZJQ0NJI1OTFMOTC"
    )


def test_get_token_suppresses_http_logging():
    """get_token() suppresses curl_cffi/urllib3 debug logging during execution."""
    import logging
    ...
    # Set debug level so we can verify it gets suppressed
    original_curl = curl_logger.level or logging.NOTSET
    original_urllib = urllib_logger.level or logging.NOTSET
    curl_logger.setLevel(logging.DEBUG)
    urllib_logger.setLevel(logging.DEBUG)

    # Build mocks for a successful flow
    certs_resp = MagicMock()
    certs_resp.status_code = 200
    certs_resp.json.return_value = {
        "retValue": {
    ...
    def check_levels_during_call():
        """Capture logger levels while inside get_token."""
        captured_levels["curl_cffi"] = logging.getLogger("curl_cffi").level
        captured_levels["urllib3"] = logging.getLogger("urllib3").level

    class CheckingSession(MagicMock):
        def get(self, *args, **kwargs):
            check_levels_during_call()
            return certs_resp
    ...
    with ExitStack() as stack:
        ...
        for p in _mock_crypto():
            stack.enter_context(p)
        get_token("user@test.com", "password", 2)

    # During execution, HTTP loggers should be suppressed to WARNING
    assert captured_levels["curl_cffi"] == logging.WARNING
    assert captured_levels["urllib3"] == logging.WARNING

    # After execution, levels should be restored
    assert curl_logger.level == logging.DEBUG
    assert urllib_logger.level == logging.DEBUG

    # Clean up
    curl_logger.setLevel(original_curl)
    urllib_logger.setLevel(original_urllib)
