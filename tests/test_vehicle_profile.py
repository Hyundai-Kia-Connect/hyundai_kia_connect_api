...
class TestMapVehicleProfile:
    ...
    def test_capability_properties_work_with_mapped_profile(self, eu_api):
        ...
        assert v.is_left_hand_drive is True
        assert v.ev_alarm_supported is False
        # 16 capabilities added from #1764 volunteer profile dumps.
        # Present flags (EV6 fixture values): non-zero -> True, "0" -> False.
        assert v.svm_supported is True  # svmOptionInfo=1
        assert v.v2l_supported is True  # V2LOption=1
        assert v.v2x_supported is False  # V2XOption=0
        assert v.frunk_supported is False  # frunkOption=0
        assert v.climate_control_supported is True  # clmtCtrlOption=1
        assert (
            v.battery_preconditioning_supported is True
        )  # batteryPreconditioningOption=2
        assert v.charge_port_door_supported is True  # chargePortDoorOption=1
        assert v.charging_current_control_supported is True  # chargingCurrentSupport=1
        assert v.v2l_soc_set_supported is True  # v2LSocSetServiceOption=1
        # Absent flags (reported only on some vehicle generations) -> None (unknown).
        assert v.v2g_supported is None  # V2GOption absent on EV6
        assert v.digital_side_mirror_supported is None  # cmsOption absent
        assert v.wireless_charging_supported is None  # wirelessChargingOption absent
        assert (
            v.remote_front_window_heat_supported is None
        )  # remoteFrontWindowHeatControl absent
        assert v.window_safety_supported is None  # windowSafetyOption absent
        assert v.window_control_supported is None  # windowControlOption absent
        assert (
            v.remote_light_control_supported is None
        )  # remoteLightControlOption absent
