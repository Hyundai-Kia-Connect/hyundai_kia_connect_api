...
@pytest.mark.integration
@pytest.mark.xfail(
    reason="EU server requires registered device_id from notifications/register; persistent UUID is rejected with resCode 4002"
)
class TestPersistentDeviceId:
    """Experiment 2: Can we use a persistent UUID like USA/CA?

    Result: NO. EU server rejects unregistered device_id with
    DeviceIDError (resCode 4002). Phase 3B (retry decorator) is
