...
class VehicleProfile:
    ...
    svm_option_info: str | None = None
    v2l_option: str | None = None
    v2x_option: str | None = None
    v2g_option: str | None = None
    frunk_option: str | None = None
    clmt_ctrl_option: str | None = None
    battery_preconditioning_option: str | None = None
    cms_option: str | None = None
    wireless_charging_option: str | None = None
    remote_front_window_heat_control: str | None = None
    window_safety_option: str | None = None
    ...
    charge_port_door_option: str | None = None
    charging_current_support: str | None = None
    v2l_soc_set_service_option: str | None = None
    window_control_option: str | None = None
    remote_light_control_option: str | None = None
    ...
    # Top-level vinInfo dispatch field ("EV5" = flat /ccs2/reservation/{charge,hvac};
    # absent on older CCSP vehicles that use combined /chargehvac).
...
class Vehicle:
    ...
    # Capability properties from VehicleProfile option/serviceOption flags.
    # None when profile or the flag is absent (unknown); non-zero = supported.

    @property
    def svm_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.svm_option_info is None
            else self.profile.svm_option_info != "0"
        )

    @property
    def v2l_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None if self.profile.v2l_option is None else self.profile.v2l_option != "0"
        )

    @property
    def v2x_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None if self.profile.v2x_option is None else self.profile.v2x_option != "0"
        )

    @property
    def v2g_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None if self.profile.v2g_option is None else self.profile.v2g_option != "0"
        )

    @property
    def frunk_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.frunk_option is None
            else self.profile.frunk_option != "0"
        )

    @property
    def climate_control_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.clmt_ctrl_option is None
            else self.profile.clmt_ctrl_option != "0"
        )

    @property
    def battery_preconditioning_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.battery_preconditioning_option is None
            else self.profile.battery_preconditioning_option != "0"
        )

    @property
    def digital_side_mirror_supported(self) -> bool | None:
        # cms = Camera Monitoring System (digital side mirrors)
        if self.profile is None:
            return None
        return (
            None if self.profile.cms_option is None else self.profile.cms_option != "0"
        )

    @property
    def wireless_charging_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.wireless_charging_option is None
            else self.profile.wireless_charging_option != "0"
        )

    @property
    def remote_front_window_heat_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.remote_front_window_heat_control is None
            else self.profile.remote_front_window_heat_control != "0"
        )

    @property
    def window_safety_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.window_safety_option is None
            else self.profile.window_safety_option != "0"
        )

    @property
    def charge_port_door_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.charge_port_door_option is None
            else self.profile.charge_port_door_option != "0"
        )

    @property
    def charging_current_control_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.charging_current_support is None
            else self.profile.charging_current_support != "0"
        )

    @property
    def v2l_soc_set_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.v2l_soc_set_service_option is None
            else self.profile.v2l_soc_set_service_option != "0"
        )

    @property
    def window_control_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.window_control_option is None
            else self.profile.window_control_option != "0"
        )

    @property
    def remote_light_control_supported(self) -> bool | None:
        if self.profile is None:
            return None
        return (
            None
            if self.profile.remote_light_control_option is None
            else self.profile.remote_light_control_option != "0"
        )
