...
class KiaUvoApiIN(ApiImplType1):
    ...
    def refresh_access_token(self, token: Token) -> Token:
        """Refresh access token using OAuth2 refresh_token grant for IN region.

        IN requires the Stamp header in oauth2/token requests, unlike AU and CN.
        """
        if token.refresh_token:
            try:
                ...
                _LOGGER.debug(
                    f"{DOMAIN} - Access token refreshed successfully (IN), "
                ...
                _LOGGER.warning(
                    f"{DOMAIN} - IN refresh token exchange failed, "
