...
class KiaUvoApiAU(ApiImplType1):
    ...
    def _force_refresh_vehicle_state_ccs2(self, token: Token, vehicle: Vehicle) -> None:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            res_msg_state = response.get("resMsg", {}).get("state", {}).get("Vehicle")
            if res_msg_state is None:
                continue

            # Check if data is fresh: compare lastUpdateTime with trigger_time
            # Date format: "20260616122326" or "20260616122326.000" (YYYYMMDDHHmmss[.ms])
            raw_date = res_msg_state.get("Date")
            if raw_date:
                try:
                    # Strip milliseconds suffix if present (e.g. ".000")
                    ...
                    _LOGGER.warning(
                        f"{DOMAIN} - CCS2 force refresh: cannot parse Date '{raw_date}'"
                    )
        ...
        if state is None:
            ...
            if response.get("retCode") == "S":
                state = response.get("resMsg", {}).get("state", {}).get("Vehicle")
